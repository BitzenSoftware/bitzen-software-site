...
export default function AgentChat({ agentId, agentLogo, onClose }) {
  ...
  async function publishToLinkedIn(text, msgIndex) {
    setPublishing(msgIndex)
    setPublishedId(null)
    try {
      ...
      if (error) throw error
      setPublishedId(msgIndex)
    } catch (e) {
      alert('Erro ao publicar no LinkedIn: ' + (e.message || 'Tente novamente'))
    } finally {
      setPublishing(null)
    }
  }
            ...
            <div key={i} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className="flex flex-col gap-1 max-w-[80%]">
                ...
                {msg.role === 'assistant' && i > 0 && (
                  <button
                    onClick={() => publishedId === i ? null : publishToLinkedIn(msg.content, i)}
                  ...
                  >
                    ...
                    {publishedId === i ? 'Publicado!' : publishing === i ? 'Publicando...' : 'Publicar no LinkedIn'}
                  </button>
                )}
              </div>
            </div>
